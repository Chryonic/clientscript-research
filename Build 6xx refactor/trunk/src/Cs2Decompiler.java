...
public class Cs2Decompiler {
    ...
    private static Stack<AstNode> longStack = new Stack<AstNode>();
    ...
    public static void decompile(Cs2Script script){
        executeScript(new Cs2Excecutable(script));
        for (Cs2DecompileState s : functions){
            try {
                PrintStream p = new PrintStream("../scripts/"+ s.getExecutable().toString()+".cs2");
                for (Cs2Variable v : s.getLocalVariables())
                    p.println(v.getType().toString().toLowerCase() + " " + v.getName() + "@" + v.getId());
                for (Cs2Variable v : globalVariables)
                    p.println("global "+ v.getType().toString().toLowerCase() + " " + v.getName() + "@" + v.getId());
                for (Cs2Variable v : configVariables)
                    p.println("config "+ v.getType().toString().toLowerCase() + " " + v.getName() + "@" + v.getId());
                p.println("\nvoid "+((Cs2Excecutable)s.getExecutable()).toString()+"("+((Cs2Excecutable)s.getExecutable()).getSignature().toParList(s)+"){");
        ...
        try {
            for (; ; ) {
                ...
                if (currentDecompileState.getLabels().containsKey(instructionCounter))
                    currentDecompileState.getAstTree().addChild(new LabelNode(
                            currentDecompileState.getLabels().get(instructionCounter)
                    ));
                ...
                if (opcode < 150) {
                    switch (opcode) {
                        ...
                        case 3:
                            ...
                            break;
                        case 6:
                            currentDecompileState.getAstTree().addChild(
                                    currentDecompileState.getTargetNode(instructionCounter + 1 + instruction.getIntOperand(),999999)
                            );
                        ...
                        case 27:
                            ...
                            vsn.addChild(integerStack.pop());
                            currentDecompileState.getAstTree().addChild(vsn);break;
                        case 31:
                            con = new InfixNode("<=");
                            conOp2=integerStack.pop();
                            con.addChild(integerStack.pop());
                        ...
                        case 40: {
                            ...
                            excecutingScript = new Cs2Excecutable(callee);
                            ...
                            for (int pV = 0;pV < excecutingScript.getSignature().getLongParameters();pV++)
                                tempStack.push(longStack.pop());
                            for (int pV = 0;pV < excecutingScript.getSignature().getLongParameters();pV++)
                                cn.addChild(tempStack.pop());
                        ...
                        case 51:
                            ...
                            break;
                        case 54:
                            longStack.push(new ConstantNode(instruction.getLongOperand()));
                            break;
                        case 55:
                            longStack.pop();
                            break;
                        case 66:
                            longStack.push(new VariableGetNode(currentDecompileState.getLocalVariable
                                    (instruction.getIntOperand(), Cs2Type.Long)));
                            break;
                        case 67:
                            vsn = new VariableSetNode(
                                    currentDecompileState.getLocalVariable(instruction.getIntOperand(), Cs2Type.Long));
                            vsn.addChild(longStack.pop());
                            currentDecompileState.getAstTree().addChild(vsn);
                            break;
                        case 68:
                            con = new InfixNode("!=");
                            conOp2=longStack.pop();
                            con.addChild(longStack.pop());
                            con.addChild(conOp2);
                            blk = new ConditionalNode();
                            blk.addChild(con);
                            blk.addChild(currentDecompileState.getTargetNode(instructionCounter + 1 + instruction.getIntOperand(),999999));
                        ...
                        case 87:
                            ...
                            break;
                        case 112:
                            BuiltinNode n = new BuiltinNode("getIntClanSetting");
                            n.addChild(new ConstantNode(instruction.getIntOperand()));
                            integerStack.push(n);
                            break;
                        case 113:
                            n = new BuiltinNode("getBitClanSetting");
                            n.addChild(new ConstantNode(instruction.getIntOperand()));
                            integerStack.push(n);
                            break;
                        case 114:
                            n = new BuiltinNode("getLongClanSetting");
                            n.addChild(new ConstantNode(instruction.getIntOperand()));
                            longStack.push(n);
                            break;
                        case 115:
                            n = new BuiltinNode("getStringClanSetting");
                            n.addChild(new ConstantNode(instruction.getIntOperand()));
                            stringStack.push(n);
                            break;
                    }
                    ...
                    if (opcode >= 150 && opcode < 5000){
                        if (opcode < 300) {
                            ...
                            if (opcode == 150) {//cc_create
                                ...
                                AstNode index = integerStack.pop();             //interfaceA = interfaceStore[id].getDynamicChild(cid)
                                AstNode idHash = integerStack.pop();
                        ...
                        /*} else if (Cs2ByteCode.isSetter(opcode)){ //if_load
                            InterfaceNode ino = new InterfaceNode(anLocalBoolean ? "interfaceA" : "interfaceB");
                            AstNode idHash = integerStack.pop();
                            InfixNode inf = new InfixNode(".");
                            BuiltinNode node = new BuiltinNode("delete");
